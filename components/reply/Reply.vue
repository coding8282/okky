<template>
  <div class="p-3 border-bottom">
    <div class="d-flex mb-2">
      ...
      <!-- 작성자 링크 -->
      <div>
        <nuxt-link :to="`/member/${reply.replierId}/profile`" class="c-normal-font mb-0" tag="h4">
          {{reply.replierName}}
        </nuxt-link>
    ...
    <textarea v-model="request.body" v-show="editing" ref="body" id="body" rows="3"
              class="form-control form-control-sm my-3" required></textarea>

    <div class="d-flex">
      <!-- 코멘트 보기 -->
      <a v-if="reply.commentCount>0" @click="onShowComments()">
        <small class="text-primary">
          코멘트 {{reply.commentCount}}개 보기
          <icon :name="showComments?'sort-down':'caret-up'" scale="1.2"/>
        </small>
      </a>
      ...
      <!--고정 -->
      <a v-if="showTogglePinButton" @click="onTogglePin()" class="ml-2">
        <small class="text-primary">
          <icon name='map-pin' :color="reply.pinned?'blue':'gray'" scale="1.2"/>
        </small>
      </a>
      <!-- 추천 -->
    ...
    methods: {
      ...
      async onTogglePin() {
        try {
          ...
          await this.$axios.$put(`/replies/${this.reply.id}/pin/toggle`);
