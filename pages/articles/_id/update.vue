<template>
  <div>
    <!-- 캡션 -->
    <h3 class="mb-0">
      <icon :name="boardId|toBoardIcon" scale="1.5"/> {{boardId|toBoardName}}
    </h3>
    ...
    <form @submit.prevent="onSubmit" class="border p-3 mt-3">
      ...
      <div class="form-group">
        <button :disabled="pendingUpdate" type="button" tag="button" class="btn btn-outline-secondary btn-sm mr-1" data-toggle="modal" data-target="#cancelModal"> 
          <icon name="undo" scale=".8"/> 취소
        </button>
        <button :disabled="pendingUpdate" class="btn btn-outline-secondary btn-sm" type="button" tag="button" data-toggle="modal" data-target="#initModal"> 
          <icon name="eraser" scale=".9"/> 원래대로
        </button>
        <button :disabled="pendingUpdate" class="btn btn-success btn-sm float-right">
          <icon name="pencil-alt" scale=".8"/> 수정
        </button>
        <icon v-if="pendingLoad||pendingUpdate" name="spinner" class="float-right mr-1 mt-1 text-muted" scale="1.3" pulse/>
      </div>
    </form>
